  ...
  listings.map((item)=>{
    return (
      <Link href={`http://localhost:3000/posts/${item.post_id}`}>
                <div className="card" >
                    <img src="https://c4.wallpaperflare.com/wallpaper/471/337/300/red-hair-girl-freckles-glasses-library-reading-book-wallpaper-preview.jpg" alt="" />
                    <h3>{ item.post_title }</h3>
                    <p> { item.post_content }</p>
                    <h3>{item.city} </h3>
                    <h3>{getLocation ? getLocation.currencySymbol : "Price is not available"} 1000</h3>
                    <h3> By {item.author.first_name} </h3>
                    <h3 style={{"color":item.category.category_color}}> In Category: {item.category.category_title} </h3>
